...
- [1. Các loại tấn công Injection phổ biến](#các-loại-tấn-công-injection-phổ-biến)  
  - [1.1. XPath Injection](#xpath-injection)  
  - [1.2. LDAP Injection](#ldap-injection)  
  - [1.3. HTML Injection trong PDF Generators](#html-injection-trong-pdf-generators)  

### **Giới thiệu về XPath Injection**  
- [2. Giới thiệu về XPath Injection](#giới-thiệu-về-xpath-injection)  
- [3. XPath Injection để Bypass Xác Thực](#xpath-injection-để-bypass-xác-thực)  
- [4. Trích xuất dữ liệu qua XPath Injection](#trích-xuất-dữ-liệu-qua-xpath-injection)  
- [5. Khai thác nâng cao để trích xuất dữ liệu](#khai-thác-nâng-cao-để-trích-xuất-dữ-liệu)  
- [6. Khai thác Blind XPath Injection](#khai-thác-blind-xpath-injection)  
- [7. Phòng chống XPath Injection và công cụ hỗ trợ khai thác](#phòng-chống-xpath-injection-và-công-cụ-hỗ-trợ-khai-thác)  

### **Giới thiệu về LDAP Injection**  
- [1. LDAP là gì?](#ldap-là-gì)  
- [2. Các loại thuộc tính phổ biến trong LDAP](#các-loại-thuộc-tính-phổ-biến-trong-ldap)  
- [3. Lỗ hổng LDAP Injection](#lỗ-hổng-ldap-injection)  
- [4. LDAP Authentication Bypass](#ldap-authentication-bypass)  
- [5. Exfiltration Dữ Liệu và Blind Exploitation với LDAP Injection](#exfiltration-dữ-liệu-và-blind-exploitation-với-ldap-injection)  
- [6. Phòng Ngừa LDAP Injection](#phòng-ngừa-ldap-injection)  
- [7. Bảo mật LDAP để tránh lỗ hổng LDAP Injection](#bảo-mật-ldap-để-tránh-lỗ-hổng-ldap-injection)  

### **Giới Thiệu Về Lỗ Hổng Tạo PDF**  
- [1. Tạo PDF và Các Lỗ Hổng Liên Quan](#tạo-pdf-và-các-lỗ-hổng-liên-quan)  
- [2. Các Thư Viện Tạo PDF Phổ Biến](#các-thư-viện-tạo-pdf-phổ-biến)  
- [3. Lỗ Hổng HTML Injection trong PDF Generation](#lỗ-hổng-html-injection-trong-pdf-generation)  
- [4. Ví Dụ: wkhtmltopdf](#ví-dụ-wkhtmltopdf)  
- [5. Phân Tích Tệp PDF](#phân-tích-tệp-pdf)  
- [6. Các Biện Pháp Phòng Ngừa Lỗ Hổng PDF Injection](#các-biện-pháp-phòng-ngừa-lỗ-hổng-pdf-injection)  
- [7. Ví Dụ Sử Dụng ExifTool](#ví-dụ-sử-dụng-exiftool)  
- [8. Các Vấn Đề Bảo Mật Tiềm Ẩn](#các-vấn-đề-bảo-mật-tiềm-ẩn)  
- [9. Các Lỗ Hổng Thường Gặp Khi Sử Dụng Thư Viện Tạo PDF](#các-lỗ-hổng-thường-gặp-khi-sử-dụng-thư-viện-tạo-pdf)  
  - [9.1. Thực Thi Mã JavaScript (Server-Side XSS)](#thực-thi-mã-javascript-server-side-xss)  
  - [9.2. Lỗ Hổng SSRF (Server-Side Request Forgery)](#lỗ-hổng-ssrf-server-side-request-forgery)  
  - [9.3. Lỗ Hổng LFI (Local File Inclusion)](#lỗ-hổng-lfi-local-file-inclusion)  
  - [9.4. Khai thác mà không cần JavaScript](#khai-thác-mà-không-cần-javascript)  
  - [9.5. Annotations và Attachments](#annotations-và-attachments)  
- [10. Ngăn ngừa các lỗ hổng trong thư viện tạo PDF](#ngăn-ngừa-các-lỗ-hổng-trong-thư-viện-tạo-pdf)  
